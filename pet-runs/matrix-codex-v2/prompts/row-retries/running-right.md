Create Codex pet row `running-right` for `matrix-codex`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the exact identity of the existing cybernetic black cat: black feline silhouette, bright Matrix-green circuitry, angular ears, luminous green eyes, compact readable sprite proportions, and its established pixel-art rendering.. Preserve silhouette, face, palette, material, proportions, markings, and props.
