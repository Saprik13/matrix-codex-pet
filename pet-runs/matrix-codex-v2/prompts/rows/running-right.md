Create one horizontal animation strip for Codex pet `matrix-codex`, state `running-right`.
...
Identity: same pet in every frame: Preserve the exact identity of the existing cybernetic black cat: black feline silhouette, bright Matrix-green circuitry, angular ears, luminous green eyes, compact readable sprite proportions, and its established pixel-art rendering.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `pixel`: Pixel-art-adjacent digital mascot with a chunky silhouette, simple dark outline, limited palette, flat cel shading, and visible stepped edges. User style notes: Match the existing atlas exactly; no redesign, no extra props, no text, no detached effects..
